...
(function ($) {
  ...
  function priceCalculation(current) {
    var price = $(current).closest('.item').find('.roomPrice').val();
    var room = $(current).closest('.item').find('.RoomDays').val();
  ...
  jQuery(document).on("change", ".SightSeeing", function (e, params) {
    ...
    var sale_price = parseFloat(beforeSalePrice)+parseFloat(total);
    $('.totalSalePrice').val(sale_price.toFixed(2));
  });
  jQuery(document).on("keyup", ".beforeSalePrice", function (e, params) {
    var sightseeingPrice = $('.sightseeingPrice').val()||0;
    var sale_price = $(this).val()||0;
    var total = parseFloat(sale_price)+parseFloat(sightseeingPrice);
    $('.totalSalePrice').val(total.toFixed(2));
  });
