...
(function ($) {
  ...
  $(".form-group-item .btn-add-item").click(function () {
    ...
    $(".timepicker").datetimepicker({
      ...
      $('.hasAppendCkEditor .appendCKEditor').each(function () {
          $(this).addClass('has-ckeditor');
      });
 setTimeout(ckEditor, 500);


  });
  jQuery(document).on("change", ".chooseExtraType", function (e, params) {
     var text = $("option:selected", this).text();
     var desc = $("option:selected", this).attr('desc');
     $(this).closest('.item').find('.title').val(text);
       $(this).closest('.item').find('.appendCKEditor').show();
      $(this).closest('.item').find('.textareaDiv .tox-tinymce').remove();
      $(this).closest('.item').find('.appendCKEditor').val(desc);
     setTimeout(ckEditor, 200);
  });
  jQuery(document).on("click", ".form-group-item .addItemMiddle", function (e, params) {
    var p = $(this).closest(".form-group-item").find(".g-items");
    var number = $(this).closest(".form-group-item").find(".g-items .item:last-child").data("number");
    if (number === undefined) number = 0;else number++;
    var extra_html = $(this).closest(".form-group-item").find(".g-more").html();
    extra_html = extra_html.replace(/__name__=/gi, "name=");
    extra_html = extra_html.replace(/__number__/gi, number);
    $(extra_html).insertAfter($(this).closest(".item"));
    var formgroupitem = $(this).closest(".form-group-item");
    ItineraryDaysCount(formgroupitem);
    var multiselectDrop2 = $(this).closest(".item").next().find('.ismultiple .multiselectDrop2');
    itinerarySightseenManage();
    checkedCheckBox();
  });
  function ItineraryDaysCount(formgroupitem) {
    if ($(formgroupitem).hasClass('ItinerarySection')) {
          var ii = 0;
          $($(formgroupitem).find(".g-items .item")).each(function(){
          var p2 = $(this).html();
          var prefix = 'itinerary['+ii+']';
            $(this).attr("data-number", ii);
            var day = ii+1;
            $(this).find('.DayNumber').val("Day "+day+"")
            $(this).find("input, textarea, select").each(function() {
               this.name = this.name.replace(/itinerary\[\d+\]/, prefix);   
            });
          ii++;
        });
    }
  ...
  jQuery(document).on("change", ".hotelLocation", function (e, params) {
    var cid = this.value;
    jQuery(this).closest('.item').find('.roomPrice').val('');
    var $el = jQuery(this).closest('.item').find('.hotelDropDown');
    var $el2 = jQuery(this).closest('.item').find('.hotelroomDropDown');
    $el.empty();
    $el2.empty();
    jQuery(this).closest('.item').find('.roomPrice').val(0);
          jQuery(this).closest('.item').find('.totalPrice').val(0);
    jQuery.ajax({
        url: bookingCore.url + '/getHotels',
        type : "POST",
        dataType : "JSON",
        data:{'location_id':cid},
        success: function(result){
          console.log(result);
          var select = "Select Hotel";
          var select2 = "Select Room";
          $el.html('<option value="">'+select+'</option>');
          $el2.html('<option value="">'+select2+'</option>');
  ...
  jQuery(document).on("change", ".hotelDropDown", function (e, params) {
    var cid = this.value;
    var $el = jQuery(this).closest('.item').find('.hotelroomDropDown');
    var start_date = jQuery(this).closest('form').find('.datePicker').val();
    $el.empty();
    jQuery(this).closest('.item').find('.roomPrice').val(0);
          jQuery(this).closest('.item').find('.totalPrice').val(0);
    jQuery.ajax({
        url: bookingCore.url + '/getRooms',
        type : "POST",
        dataType : "JSON",
        data:{'hotel_id':cid,'start_date':start_date},
        success: function(result){
          console.log(result);
  ...
  jQuery(document).on("change", ".hotelroomDropDown", function (e, params) {
    jQuery(this).closest('.item').find('.roomPrice').val(0);
          jQuery(this).closest('.item').find('.totalPrice').val(0);
    var cid = this.value;
    var price = $('option:selected', this).attr('price')||0;
    jQuery(this).closest('.item').find('.roomPrice').val(price);
    priceCalculation(this);
  });
  jQuery(document).on("change", ".RoomDays", function (e, params) {
    priceCalculation(this);
  });
  function priceCalculation(current) {
    var price = $(current).closest('.item').find('.roomPrice').val()||0;
    var room = $(current).closest('.item').find('.RoomDays').val()||0;
    var total = parseInt(price) * room;
    $(current).closest('.item').find('.totalPrice').val(total);
    totalHotelPrice();
  }
  function totalHotelPrice() {
    var total = 0;
    $(".defaultHotels .TotalDiv .totalPrice").each(function(){
      var roomPrice = $(this).val()||0;
      total += parseFloat(roomPrice);
    });
    $('.totalHotelPrice').val(total);
    salePriceCalculation();
  }
  function salePriceCalculation() {
    var sightseeingPrice = $('.sightseeingPrice').val()||0;
    var transfersPrice = $('.transfersPrice').val()||0;
    var totalHotelPrice = $('.totalHotelPrice').val()||0;
    var beforeSalePrice = $('.beforeSalePrice').val()||0;
    var sale_price = parseFloat(beforeSalePrice)+parseFloat(totalHotelPrice)+parseFloat(sightseeingPrice)+parseFloat(transfersPrice);
    $('.totalSalePrice').val(sale_price.toFixed(2));
    // $('.totalPersonPrice').attr("value",sale_price.toFixed(2));
    $(".totalPersonPrice").each(function(){
      var personName = $(this).closest('.item').find('.personName').val();
      if (personName == 'Child' || personName == 'child') {
        sale_price = sale_price-(parseFloat(totalHotelPrice)/2);
      }
      $(this).val(sale_price);
      // alert(new_price);
    });
    ...
    jQuery(document).on("click", ".refreshTour", function (e, params) {

     $(".g-items .item .hotelroomDropDown").each(function(){
        $(this).change();
      });
     priceCalculateCheckBox();
     totalHotelPrice();
    alert('done');
  });
  jQuery(document).on("change", ".SightSeeing", function (e, params) {
    priceCalculateCheckBox();
  });

  function priceCalculateCheckBox() {
    var total = 0;
    var totalTransfer = 0;
    $(".SightSeeing").each(function(){
      if ($(this).is(":checked")) {
        var attrPrice = $(this).attr('price')||0;
        total += parseFloat(attrPrice);
    ...
    // if ($(this).is(":checked") && price != "") {
    //   var total = parseFloat(sightseeingPrice)+parseFloat(price);
    // }else{
    //   var total = parseFloat(sightseeingPrice)-parseFloat(price);
    // }
    console.log('sightseeingPrice',total);
    console.log('transfersPrice',totalTransfer);
    $('.sightseeingPrice').val(total.toFixed(2));
    $('.transfersPrice').val(totalTransfer.toFixed(2));
  }
  jQuery(document).on("keyup", ".beforeSalePrice", function (e, params) {
    salePriceCalculation();
  });
  ...
  jQuery(document).on("keydown", ".enter-mail-id", function (e, params) {
    if (this.value != "" && (e.keyCode == 13 || e.keyCode == 32)) {
     var getValue = $(this).val();
     if (validateEmail(getValue)) {
       $('.all-mail').append('<span class="email-ids">'+ getValue +' <span class="cancel-email">x</span><input type="hidden" value="'+getValue+'" name="emails[]"></span>');
       $(this).val('');
     }
     else{
  ...
  $('#attributeTermForm').on('keyup keypress', function(e) {
  ...
  jQuery(document).on("change", "#documentsUpload", function (e, params) {
    previewFiles(this);
  });
function previewFiles() {
  ...
  function readAndPreview(file) {
    ...
    if ($.inArray(ext, ['png','jpg','jpeg','pdf']) >=0) {
      var pdf_icon = bookingCore.url+'/uploads/pdf_icon.png';
      var reader = new FileReader();
        reader.onload = function (e) {
          ...
          var li = '<li class="new"><img id="blah" width="100" src="'+doc+'" alt="your image" /></li>';
          $('#adminDoc').append(li);
        };
      reader.readAsDataURL(file);
    }else{
      alert("."+ext+" format are not allowed");
      $('input[type=file]').val('')
    }
  ...
  if (files) {
    $('#adminDoc').find('.new').remove();
    [].forEach.call(files, readAndPreview);
  }

}
jQuery(document).on("click", ".fileDelete", function (e, params) {
    if(confirm("Are you sure you want to delete this?")){
      $(this).closest('li').remove();
    }
});
jQuery(document).on("change", ".voucher_type", function (e, params) {
  // $("input[name='term_id']").val('');
    if (this.value == "2") {
      ...
      $("select[name='transport']").prop('selectedIndex',0);
      $("select[name='transport']").removeAttr("required");
      
      $("select[name='sightseeing']").prop('selectedIndex',0);
      $("select[name='sightseeing']").removeAttr("required");

      $("select[name='hotels']").prop('selectedIndex',0);
      $("select[name='hotels']").removeAttr("required");

      $("select[name='restaurant']").prop('required',true);
      $('#Restaurant').show();
      $('#nonHotelDiv').show();
      $('.package_details').val('Breakfast, Lunch, Dinner');
      $("textarea[name='remark']").val('Restaurants transfer not included customer reach restaurant by walk and own transfer.');
    }
      ...
      $("select[name='restaurant']").prop('selectedIndex',0);
      $("select[name='sightseeing']").prop('selectedIndex',0);

      $("select[name='sightseeing']").removeAttr("required");
      $("select[name='restaurant']").removeAttr("required");

      $("select[name='hotels']").prop('required',true);
      $('#Hotels').show();
      $('#HotelDiv').show();
      $('.package_details').val('');
      ...
      $("select[name='transport']").prop('required',true);
      $('#Transportation').show();
      $('#nonHotelDiv').show();
      $('.package_details').val('');
      $("textarea[name='remark']").val('');
      ...
      $("select[name='sightseeing']").prop('required',true);
      $('#Sightseeing').show();
       $('#nonHotelDiv').show();
      $('.package_details').val('');
      $("textarea[name='remark']").val('');
...
jQuery(document).on("change", "select[name='hotels']", function (e, params) {
  var room_type = $('option:selected', this).attr('room_type');
  var cDate = $('option:selected', this).attr('cDate');
  var coDate = $('option:selected', this).attr('coDate');
  var rooms = $('option:selected', this).attr('rooms');
  $('.roomType').val(room_type);
  $('.check_inDate').val(cDate);
  $('.check_outDate').val(coDate);
  $('.no_of_room').val(rooms);
});


jQuery(document).on("change", "select[name='sightseeing']", function (e, params) {
  var cDate = $('option:selected', this).attr('date');
  var coDate = $('option:selected', this).attr('time');
  $('.datePicker').val(cDate);
  $('.timepicker').val(coDate);
});
jQuery(document).on("change", "select[name='transport']", function (e, params) {
  var cDate = $('option:selected', this).attr('date');
  var coDate = $('option:selected', this).attr('time');
  $('.datePicker').val(cDate);
  $('.timepicker').val(coDate);
  ...
  jQuery(document).on("change", ".tourCoupons", function (e, params) {
    var value = this.value;
    var current = this;
    $(".tourCoupons").not(this).each(function(){
      if ($(this).val() != "" && $(this).val() == value) {
        alert('Coupon Alredy selected');
        $(current).prop('selectedIndex',0);
         // $('#countrylist').val('').trigger('change');
        return false;
      }
    });
    jQuery(this).closest('.item').find('.discount_type').val('');
    jQuery(this).closest('.item').find('.discount').val('');
    jQuery(this).closest('.item').find('.code').val('');

    var discount_type = $('option:selected', this).attr('discount_type');
    var discount = $('option:selected', this).attr('discount');
    var code = $('option:selected', this).attr('code');

    jQuery(this).closest('.item').find('.discount_type').val(discount_type);
    jQuery(this).closest('.item').find('.discount').val(discount);
    jQuery(this).closest('.item').find('.code').val(code);

    jQuery(this).closest('.item').find('.roomPrice').val(price);
    priceCalculation(this);
  });

jQuery(document).on("change", ".TourChange", function (e, params) {
  $('.submitBookingProposal').click();
  // var url = $("input[name='proposalRoute']").val();
  // var redirectUrl = url+'/'+this.value;
  // window.location.href = redirectUrl;
});
// jQuery(document).on("change", ".proposalHotelRoom", function (e, params) {
//   var me = this;
//   setTimeout(function(){

//   var default_hotel = JSON.parse($(me).closest('.item').find('.hotelJson').val());
//   var roomPrice = $($(me).closest('.item').find('.hotelroomDropDown')).find('option:selected').attr('price')||0;
//   var RoomDays = $(me).closest('.item').find('.RoomDays').val()||0;
//   var changeRoomPrice = parseFloat(roomPrice) * parseFloat(RoomDays);
//   var modifyRoomPrice = parseInt(changeRoomPrice) - parseInt(default_hotel.total_price)
//   $(me).closest('.item').find('.modify_is').val('true');
//   var sign = "+";
//   if (modifyRoomPrice < 0) {
//     $(me).closest('.item').find('.modify_is').val('false');
//     sign = "";
//   }
//   $(me).closest('.item').find('.diff_price').val(modifyRoomPrice);
//   $(me).closest('.item').find('p .modifyPrice').text(sign+''+modifyRoomPrice);
// }, 1000);
// });
jQuery(document).on("keyup change", ".bookingProposalGuest", function (e, params) {
  var priceOnePerson = $(this).closest('.item').find('.priceOnePerson').val()||0;
  var totalPrice = parseFloat(priceOnePerson) * this.value;
  $(this).closest('.item').find('.totalPrice').val(totalPrice);
  // var modifyPrice = $(this).closest('.item').find('.modifyPrice').attr('data-id');
  // modifyPrice = parseFloat(modifyPrice) * this.value;
  // if (priceOnePerson > 0) {
  //   $(this).closest('.item').find('.modifyPrice').text(modifyPrice);
  // }
  proposalTourCost();
});
jQuery(document).on("change", ".bookingProposalExtra", function (e, params) {
  proposalTourCost();
});
...
function proposalTourCost() {
  ...
  var totalGuest = 0;
  $(".PersonTypes .item").each(function(){
    var guestPrice =  $(this).find('.totalPrice').val()||0;
    var guest =  $(this).find('.bookingProposalGuest').val()||0;
    totalGuestPrice += parseFloat(guestPrice);
    totalGuest += parseFloat(guest);
  });
  $(".extra_price_item").each(function(){
    if ($(this).find('.bookingProposalExtra').is(":checked")) {
      var extraPrice = $(this).find('.totalPrice').val()||0;
      extraPrice = parseFloat(extraPrice) * parseFloat(totalGuest);
      totalExtraPrice += parseFloat(extraPrice);
    }
  });
  var praposalDiscount = $('.TotalPriceSection .PraposalDiscount').val()||0;

  $('.TotalPriceSection .TotalPrice').val(totalGuestPrice);
  $('.TotalPriceSection .TotalExtraPrice').val(totalExtraPrice);
  if (praposalDiscount > 0) {
    totalGuestPrice = parseFloat(totalGuestPrice) + parseFloat(Math.abs(praposalDiscount));
  }else {
    totalGuestPrice = parseFloat(totalGuestPrice) - parseFloat(Math.abs(praposalDiscount));
  }
  $('.TotalPriceSection .proposalSalePrice').val(totalGuestPrice+totalExtraPrice);
}
  jQuery(document).on("change", ".DestinationChange", function (e, params) {
    var cid = this.value;
    var $el = jQuery('.TourChange')
    var $e2 = jQuery('#multiple-checkboxes')
    var $e3 = jQuery('.DurationChange')
    $el.empty();
    $e2.empty();
    $e3.empty();
    jQuery.ajax({
        url: bookingCore.url + '/getToursByLocation',
        type : "POST",
        dataType : "JSON",
        data:{'location_id':cid,'type':'location'},
        success: function(result){
          var select = "Select Package";
          var select2 = "Select Sightseeing";
          var select3 = "Select Duration";
          $el.html('<option value="">'+select+'</option>');
          jQuery.each(result.tours,function(index, element) {
              $el.append(jQuery('<option>', {
                 value: element.id,
                 text : element.title
              }));
          });
          var attributesss = result.attributes;
          
          $e2.html('<option value="">'+select2+'</option>');
          jQuery.each(attributesss,function(index, element) {
            jQuery.each(element.child,function(index, element2) {
              var name = element2.name+' ('+element.parent.name+')';
              $e2.append(jQuery('<option>', {
                 value: element2.id,
                 text : name
              }));
            });
          ...
          $e3.html('<option value="">'+select3+'</option>');
          jQuery.each(result.durations,function(index, element) {
              $e3.append(jQuery('<option>', {
                 value: element.id,
                 text : element.name
              }));
          });
   ...
   $(document).ready(function() {
        $('.CustomTour').find('.panel .panel-body .form-group').hide();
        $('.CustomTour').find('.panel .panel-body .form-group-item').hide();
        $('.CustomTour').find('.panel .panel-body .ItinerarySection ').show();
        $('#multiple-checkboxes').multiselect({
          ...
          onChange: function(element, checked) {
            ...
            var $el = jQuery('.TourChange')
            var location_id = jQuery('.DestinationChange').val();
            $el.empty();
            jQuery.ajax({
              url: bookingCore.url + '/getToursByLocation',
              type : "POST",
              dataType : "JSON",
              data:{'location_id':location_id,'sightseeings':selected,'type':'sightseeing'},
              success: function(result){
                var select = "Select Package";
                $el.html('<option value="">'+select+'</option>');
                jQuery.each(result,function(index, element) {
...
//    jQuery(document).on("change", ".SightseeingChange", function (e, params) {
//   // var url = $("input[name='proposalRoute']").val();
//   // var redirectUrl = url+'/'+this.value;
//   // window.location.href = redirectUrl;
// });
jQuery(document).on("change", ".tourAttributes", function (e, params) {
  $("input[name='term_id']").val(this.value);
});
  jQuery(document).on("change", ".getSightseenLocation", function (e, params) {
    
    var cid = this.value;
    var $el = jQuery(this).closest('.item').find('.itinerarySightseen');
     $el.empty();
    var multiselectDrop = jQuery(this).closest('.item').find('.itinerarySightseen');
    // $el.empty();
    jQuery.ajax({
        url: bookingCore.url + '/getTermsByAttr',
        type : "POST",
        dataType : "JSON",
        data:{'id':cid},
        success: function(result){
        ...
        $(multiselectDrop).addClass('multiselectDrop');
    ...
    // jQuery(document).on("change", ".itinerarySightseen", function (e, params, data) {
    //   var multiselectDrop = $('.itinerarySightseen');
    //   var arrays = [];
    //    $(".itinerarySightseen").each(function(){
    //     var thisAllValues =  $(this).val();
    //       jQuery.each(thisAllValues, function(index, item) {
    //           arrays.push(item);
    //       });
    //       // console.log(arrays);
    ...
    //              if(jQuery.inArray(this.value, arrays) !== -1){
    ...
    //     multiselectDrop.select2();
        
    //   // console.log('event', e);
    //   // console.log($(this).select2('data'));
    // });

    function itinerarySightseenManage() {
      var multiselectDrop = $('.itinerarySightseen');
      var arrays = [];
       $(".uniqueDropDown").each(function(){
        ...
        $(".uniqueDropDown").each(function(){
            var thiss = $(this)[0];
            var length = $('option:not(:selected)', this).length;
            if (length > 0) {
              $('option:not(:selected)', this).each(function() {
                // if($(this).not(':selected')){
                 if(jQuery.inArray(this.value, arrays) !== -1){
    ...
    //   var selected_values = $(e.currentTarget).val();
    //   console.log(selected_values);
    //   var select_val = e.params.data;
    //   console.log(e);
    // });
  // jQuery(document).on("change", ".changeTermByClick", function (e, params) {
  //   var thisVal = this.value;
  //   addItineraryDesc(this);
  //   var text = $('option:selected', this).text();
  //   $(".terms-scrollable .term-item input").each(function(){
  //     if (thisVal == $(this).val()) {
  //       $(this).trigger('click');
  //     }
  //   });
...
jQuery(document).on("change", ".commonMultiDrop", function (e, param, values) {
  ...
  multiselectChange(element, data, checked);
  checkedCheckBox();
});
$(".commonMultiDrop").on("select2:unselecting", function (e, param, values) {

  if (jQuery.isEmptyObject(e.params.args) == false) {
...
function multiselectChange(element, data, checked) {
  // console.log(data);
    jQuery.each(data, function(index, item) {
        var val = item;
        if (val != "") {
          var termSequence = $(element).closest('.item').find('.termSequence').val();
          if (checked == true) {
              if (termSequence != "") {
                var selected = JSON.parse(termSequence);
              }else{
                var selected = [];
              }
  ...
  //         // $(element).closest('select').css({"color": "red", "border": "2px solid red"});
  //         var thisElement = $(element).closest('select')
  //         var options = $('option:selected', thisElement);
  //         var checkBox = $(".terms-scrollable .term-item input[value='"+val+"']");
...
function checkedCheckBox() {
  // $('.terms-scrollable .term-item input:checkbox').prop('checked', false);
  $('.terms-scrollable .term-item .SightSeeing:checked').each(function() {
     $(this).prop('checked', false);
  });


  setTimeout(function(){ 
      $(".commonMultiDrop option").each(function(){
       if($(this).is(':selected')) {
         var checkBox = $(".terms-scrollable .term-item input[value='"+this.value+"']");
         if (checkBox) {
          $(checkBox).prop('checked', true);
          // console.log(checkBox.attr('transfer'), checkBox.next().text());
         }
       }
      }); 
    },500);
  ...
  function addItineraryDesc(thisEvent) {
    var DescTerms =  $(thisEvent).closest('.item').find('.ismultiple');
    var text = '';
    var desc = '';
    var termSequence = $(thisEvent).closest('.item').find('.termSequence').val();
    if (termSequence != "") {
      termSequence = JSON.parse(termSequence);
      jQuery.each(termSequence,function(k, v) {
        var thisOption = $(DescTerms).find('select option[value="'+v+'"]');
        if ($(thisOption).text() != "") {
          text += $(thisOption).text()+', ';
          if ($(thisOption).attr('desc') != "") {
            desc += $(thisOption).text()+':-'+$(thisOption).attr('desc')+'\n';
          }
        }
      });
    }

    // $(DescTerms).each(function(){
    //   var termSequence = $(thisEvent).closest('.item').find('.termSequence').val();
    //   var thisDrop = $(this);
    //   var selectedLenght = $(this).find('select option:selected').length;
    //   if (termSequence != "" && selectedLenght > 0) {
    //       termSequence = JSON.parse(termSequence);
    //       jQuery.each(termSequence,function(k, v) {
    //         var thisOption = $(thisDrop).find('select option[value="'+v+'"]');
    //         if ($(thisOption).text() != "") {
    //           text += $(thisOption).text()+', ';
    //           if ($(thisOption).attr('desc') != "") {
    //             desc += $(thisOption).text()+':-'+$(thisOption).attr('desc')+'\n';
    ...
    $(thisEvent).closest('.item').find('.itineraryDesc').val('');
    $(thisEvent).closest('.item').find('.itineraryDesc').val(text);
    
    $(thisEvent).closest('.item').find('.full-h').val('');
    $(thisEvent).closest('.item').find('.full-h').val(desc);
  }
  /**--------------------------------------------------------------------------------------
 * [LEADS - KANBAN] 
        ...
        var cardsDraggable = dragula({
            ...
            .on('dragend', function (card) {
                ...
                var update_position = $.ajax({
                    ...
                    type: 'post',
                    url: bookingCore.url + '/admin/module/Lead/changeLeadStatus',
                    dataType: 'json',
                    data:{'id':this_task_id,'status':board_name},
                });
            });
    }
}
...
jQuery(document).on("click", "#tasks-view-wrapper .board .viewLead", function (e, params) {
   var this_task_id = $(this).closest('.kanban-card').attr('data-task-id');
       $.ajax({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        ...
        dataType: 'HTML',
        data:{'id':this_task_id},
        success: function(data){
          $('#viewleadInfoModal').html(data);
          $('#viewleadInfoModal').modal('show');
        }
     });
});
...
jQuery(document).on("click", ".cancelLead", function (e, params) {
   ...
   if(action == 'cancel'){
       $.ajax({
        ...
        url: bookingCore.url + '/admin/module/Lead/changeLeadStatus',
        dataType: 'json',
        data:{'id':this_task_id,'status':'cancel'},
        success: function(data){
            $(current).closest('.kanban-card').slideUp();
            setTimeout(function(){ $(current).closest('.kanban-card').remove(); }, 3000);
        }
     });
   }
...
jQuery(document).on("click", ".switchTab", function (e, params) {
   ...
   var id = $(this).attr('data-id');
    $.ajax({
        ...
        type: 'post',
        url: bookingCore.url + '/admin/module/Lead/switchTab',
        dataType: 'HTML',
        data:{'id':id,'tab':tab,'phone':phone},
        success: function(data){
          if (tab == 'reminder') {
            $('#tab4').html(data);
          }
            ...
            $('#tab7').find('.activity-feed').html(data);
          }
        }
    });
});
jQuery(document).on("click", ".closeReminder", function (e, params) {
     $('#myModal2').modal('hide');
});
jQuery(document).on("change", ".fatchFlight", function (e, params) {
   var airport_from = jQuery(this).closest('.item').find('.from_airport').val();
   var airport_to = jQuery(this).closest('.item').find('.to_airport').val();
    var $el = jQuery(this).closest('.item').find('.flights');
    $el.empty();
    jQuery.ajax({
        url: bookingCore.url + '/getFlights',
        type : "POST",
        dataType : "JSON",
        data:{'airport_from':airport_from,'airport_to':airport_to},
        success: function(result){
          console.log(result);
          var select = "Select Flight";
          $el.html('<option value="">'+select+'</option>');
          jQuery.each(result,function(index, element) {
            $el.append(jQuery('<option>', {
               value: element.id,
