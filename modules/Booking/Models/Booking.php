...
class Booking extends BaseModel
{
    ...
    {
        $tour_summary = json_decode($this->tour_summary, true);
        ...
        foreach ($tour_summary as $key => $summary) {
            ...
            if ($key > 0) {
                $breackfast = 'Breakfast at Hotel Restaurant';
                $rowspan += 1;
            }
            ...
            if (!empty($summary['hotel']) && count($summary['hotel']) > 0) {
                ...
                if ($summary['hotel']['hotel'] != $default_hotel['hotel']) {
                    $summary['hotel']['hotel'] = $default_hotel['hotel'];
                     $summary['hotel']['hotel_img'] = $default_hotel['hotel_img'];
                     $summary['hotel']['hotel_name'] = $default_hotel['hotel_name'];
                     $summary['hotel']['room'] = $default_hotel['room'];
                     $summary['hotel']['room_name'] = $default_hotel['room_name'];
                     $summary['hotel']['room_price'] = $default_hotel['room_price'];
    ...
    public function allActivities(){
        ...
        foreach ($tour_summary as $key => $summary) {
            ...
            if (!empty($summary['hotel']) && count($summary['hotel']) > 0) {
                 $summary['hotel']['date'] = $summary['date'];
                 $activities[] = $summary['hotel'];
    ...
    public function ManageItineraryStatus()
    {
        //if(isset($this->cachedMeta[$key])) return $this->cachedMeta[$key];
        $val = DB::table('manage_itinerary')->where([
            'booking_id' => $this->id,
            'status'       => "publish"
        ])->first();
        if (!empty($val)) {
