...
?>
    <div class="container">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{ __('Lead Call Logs')}}</h1>

        </div>
        @include('admin.message')
        <div class="filter-div d-flex">
            <div class="col-left">
               <form method="get" action="{{url('/admin/module/Lead/lead-call-logs')}} " class="filter-form filter-form-right d-flex justify-content-end" role="search">
                   <div class="row">
                    ...
                    <table class="table table-hover">
                        ...
                        @if(count($result['data']['data']) > 0)
                            ...
                            <tr>
                                ...
                                <td>{{date('H:i A', $row['startTime'])}} <strong>({{sec_to_time($row['duration'])}})</strong></td>
                                <td>{{$row['status']}}</td>
                                <td>{{$row['type']}}</td>
                                 <td>{{$row['calledBy']}}</td>
                                <td>{{date('d/m/Y', $row['createdAt'])}} </td>
                                <td><button class="btn btn-info viewleadInfoModal" data-phone="{{$row['phoneNumber']}}"><i class="fa fa-eye"></i></button></td>
                            </tr>
                            @endforeach
                        @endif
                        </tbody>
