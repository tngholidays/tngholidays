...
use Modules\Sms\Core\Drivers\TradelitDriver;
use Modules\Sms\Core\Drivers\WhatsAppDriver;
class SmsManager extends Manager
{
    ...
    public function createTradelitDriver()
    {
        \config()->set('sms.tradelit.from',setting_item('sms_tradelit_api_from',\config('sms.tradelit.from')));
        \config()->set('sms.tradelit.token',setting_item('sms_tradelit_token',\config('sms.tradelit.token')));
        return new TradelitDriver(
            $this->app['config']['sms.tradelit']
        );
    }
    public function createWhatsAppDriver()
    {
        
        \config()->set('sms.whatsapp.token',setting_item('sms_whatsapp_token',\config('sms.whatsapp.token')));
        return new WhatsAppDriver(
            $this->app['config']['sms.whatsapp']
        );
    }
