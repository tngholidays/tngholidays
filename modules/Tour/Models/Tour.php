...
class Tour extends Bookable
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $default_hotels_input = $request->input('default_hotels');
        ...
        $booking->applied_coupon = json_encode($applied_coupon);
        $booking->proposal_discount = $proposal_discount;
    ...
    public function addToCartByAdmin(Request $request)
    {
        ...
        // Add Booking
        // get Price Availability Calendar
        $dataPriceAvailability = $tour->getDataPriceAvailabilityInRanges($request->input('start_date'));

        $total = 0;
        $total_guests = 0;
        $discount = 0;
        $base_price = ($tour->sale_price and $tour->sale_price > 0 and $tour->sale_price < $tour->price) ? $tour->sale_price : $tour->price;
        // for Availability Calendar
        $base_price = $dataPriceAvailability['base_price'] ?? $base_price;
        $extra_price = [];
        $extra_price_input = $request->input('extra_price');
        $discount_by_people_input = $request->input('discount_by_people');

        $custom_tour = $request->input('custom_tour');
        $person_types = [];
        $person_types_input = $request->input('person_types');
        $default_hotels_input = $request->input('default_hotels');
        $applied_coupon = $request->input('applied_coupon');
        ...
        $meta = $tour->meta;
        $attributes = json_encode($tourDetails['terms']);
        ...
        if ($meta) {
                        ...
                        if (isset($extra_price_input[$k]) and $extra_price_input[$k]['enable'] and $extra_price_input[$k]['enable'] != 'false') {
                            ...
                            switch ($type['type']) {
                                case "one_time":
                                    ...
                                    break;
                                case "per_hour":
                                    $type_total = $type['price'] * $tour->duration;
                                    break;
                                case "per_day":
                                    $type_total = $type['price'] * ceil($tour->duration / 24);
                                    break;
                            }
                            if (!empty($type['per_person'])) {
                                $type_total *= $total_guests;
            ...
            // $attributesss = getTermsById($tour_term);

            $transfers_price = $custom_tour['transfers_price'] * $total_guests; 

            if ($discount_by_people_input and !empty($discount_by_people_input)) {
                foreach ($discount_by_people_input as $type) {
                    if ($type['from'] <= $total_guests and (!$type['to'] or $type['to'] >= $total_guests)) {
        ...
        $booking->applied_coupon = json_encode($applied_coupon);
        $booking->proposal_discount = $proposal_discount;
        $booking->proposal_id = $proposal_id;

        $booking->calculateCommission();
        if ($tour->isDepositEnable()) {
            $booking_deposit_fomular = $tour->getDepositFomular();
            $tmp_price_total = $booking->total;
            if ($booking_deposit_fomular == "deposit_and_fee") {
                $tmp_price_total = $booking->total_before_fees;
            }
            switch ($tour->getDepositType()) {
                case "percent":
                    $booking->deposit = $tmp_price_total * $tour->getDepositAmount() / 100;
                    break;
                default:
                    $booking->deposit = $tour->getDepositAmount();
                    break;
            }
            if ($booking_deposit_fomular == "deposit_and_fee") {
                $booking->deposit = $booking->deposit + $total_buyer_fee + $total_service_fee;
        ...
        $check = $booking->save();
        if ($check) {
            $tour->bookingClass::clearDraftBookings();
            $booking->addMeta('duration', $tour->duration);
            $booking->addMeta('base_price', $base_price);
            $booking->addMeta('guests', max($total_guests, $request->input('guests')));
            $booking->addMeta('extra_price', $extra_price);
            $booking->addMeta('person_types', $person_types);
            ...
            if ($tour->isDepositEnable()) {
                $booking->addMeta('deposit_info', [
                    ...
                    'amount'  => $tour->getDepositAmount(),
                    'fomular' => $tour->getDepositFomular(),
                ]);
            }
            return $tour->sendSuccess([
                'url'          => $booking->getCheckoutUrl(),
                'booking_code' => $booking->code,
            ]);
        }
    ...
    public function getBookingData()
    {
        $booking_data = [
            ...
            'open_hours'      => [],
            'extra_price'     => [],
            'minDate'         => date('m/d/Y' ,strtotime(date('m/d/Y') . ' +4 day')),
        ...
        if ($meta) {
            ...
            $oldRoomPrice = 0;
            $updatedRoomPrice = 0;
            if ($meta->enable_default_hotels) {
                $booking_data['default_hotels'] = $meta->default_hotels;
                if (!empty($booking_data['default_hotels'])) {
                    foreach ($booking_data['default_hotels'] as $k => &$type) {
                    $oldRoomPrice += $type['total_price'];
                    $room = getRoomsById($type['room']);
                ...
                $booking_data['default_hotels'] = array_values((array)$booking_data['default_hotels']);
                $booking_data['by_default_hotels'] = array_values((array)$booking_data['default_hotels']);
            }
            if ($meta->enable_person_types) {
                $booking_data['person_types'] = $meta->person_types;
                if (!empty($booking_data['person_types'])) {
                    foreach ($booking_data['person_types'] as $k => &$type) {
                        $perPersonPrice = ($type['price']-$oldRoomPrice)+$updatedRoomPrice;
                        if (!empty($lang) and !empty($type['name_' . $lang])) {
                        ...
                        $type['price'] = $perPersonPrice;
                        $type['display_price'] = format_money($perPersonPrice);
                    }
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
    ...
    {
        $res = "";
        if (!empty($this->meta->default_hotels)) {
            foreach ($this->meta->default_hotels as $key => $hotel) {
                $location = @getLocationById($hotel['location_id'])->name;
                $days = $hotel['days'] * 2;
                $res .= $location.' '.$days.'N';
                if (count($this->meta->default_hotels) != $key+1) {
                    $res .= ", ";
                }
            }
        }
        return $this->attributes['tour_duration'] = $res;
    }
