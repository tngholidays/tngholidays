...
//        for ($i = strtotime($request->query('start')); $i <= strtotime($request->query('end')); $i += DAY_IN_SECONDS) {
        ...
        foreach ($period as $dt){
            ...
            $oldRoomPrice = 0;
            $updatedRoomPrice = 0;
            if (!empty($tour->meta->enable_default_hotels) and $tour->meta->enable_default_hotels == 1) {
                $date['default_hotels'] = $tour->meta->default_hotels;
                $default_hotels = array();
                foreach ($date['default_hotels'] as $k => $hotel) {
                    $oldRoomPrice += $hotel['total_price'];
                    $room = getRoomsById($hotel['room']);
                    $roomBookings = $room->getCustomDatesInRange(date('Y-m-d', $i),'','single');
                    $hotelDDetail = getHotelById($hotel['hotel']);
                    $hotel['location_name'] = getLocationById($hotel['location_id'])->name;
                    $hotel['hotel_name'] = $hotelDDetail->title;
                    $hotel['hotel_img'] = url('/uploads').'/'.getImageUrlById($hotelDDetail->image_id);
                    $hotel['room_name'] = $room->title;
                    $hotel['room_price'] = $room->price;
                    $hotel['total_price'] = $room->price*$hotel['days'];
                    $hotel['price_html'] = format_money($room->price * $hotel['days']);
                    $updatedRoomPrice += $hotel['total_price'];
                    $default_hotels[] = $hotel;
                }
                $date['default_hotels'] = $default_hotels;
            }
            if (!empty($tour->meta->enable_person_types) and $tour->meta->enable_person_types == 1) {
                ...
                if (!empty($date['person_types'])) {
                    ...
                    foreach ($date['person_types'] as &$person) {
                        $perPersonPrice = ($person['price']-$oldRoomPrice)+$updatedRoomPrice;
                        if (!$is_single) {
                            $c_title .= $person['name'] . ": " . format_money_main($perPersonPrice) . "<br>";
                            //for single
                            $person['display_price'] = format_money_main($perPersonPrice);
                        } else {
                            $c_title .= $person['name'] . ": " . format_money($perPersonPrice) . "<br>";
                            //for single
                            $person['display_price'] = format_money($perPersonPrice);
                        }
            ...
            foreach ($rows as $row) {
                ...
                if (!empty($tour->meta->enable_default_hotels) and $tour->meta->enable_default_hotels == 1) {
                    ...
                    foreach ($date['default_hotels'] as $k => $hotel) {
                        $oldRoomPrice += $hotel['total_price'];
                        $room = getRoomsById($hotel['room']);
                        $roomBookings = $room->getCustomDatesInRange(date('Y-m-d', strtotime($row->start_date)),'','single');
                        $hotelDDetail = getHotelById($hotel['hotel']);
                        $hotel['location_name'] = getLocationById($hotel['location_id'])->name;
                        $hotel['hotel_name'] = $hotelDDetail->title;
                        $hotel['hotel_img'] = url('/uploads').'/'.getImageUrlById($hotelDDetail->image_id);
                ...
                $row->person_types = $default_hotels;
                ...
                if (!empty($tour->meta->enable_default_hotels) and $tour->meta->default_hotels == 1) {
                    $date['default_hotels'] = $tour->meta->default_hotels;
