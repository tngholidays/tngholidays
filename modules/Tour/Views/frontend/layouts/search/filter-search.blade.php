...
<?php /*
        ...
        @endphp
        @php
            $attributes = \Modules\Core\Models\Terms::getTermsById($attributesIds);
        @endphp
        @foreach ($attributes as $item)
            @php
                $translate = $item['parent']->translateOrOrigin(app()->getLocale());
            @endphp
            @if($item['parent']->is_filter == 1)
            <div class="g-filter-item">
                <div class="item-title">
                ...
                <div class="item-content">
                    <ul>
                        @foreach($item['child'] as $key => $term)
                    ...
                    @if(count($item['child']) > 3 and empty($selected))
                        <button type="button" class="btn btn-link btn-more-item">{{__("More")}} <i class="fa fa-caret-down"></i></button>
                    @endif
                </div>
            </div>
