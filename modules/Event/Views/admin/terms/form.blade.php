...
<div class="form-group">
    <label>{{__("Type")}}</label>
    <?php $getExtraContentTypes = getExtraContentTypes() ?>
     <select name="type" class="form-control">
        <option value="">{{__("-- Please Select --")}}</option>
        @if(count($getExtraContentTypes) > 0)
            @foreach($getExtraContentTypes as $extra_key => $extra )
                <option value="{{$extra_key}}" {{$translation->type == $extra_key ? "selected" : ""}}>{{$extra}}</option>
            @endforeach
        @endif
    </select>
</div>
...
<div class="form-group">
    ...
    <textarea name="desc" rows="3" class="form-control d-none has-ckeditor" placeholder="Enter description...">{{$translation->desc}}</textarea>
</div>
@if(is_default_lang())
