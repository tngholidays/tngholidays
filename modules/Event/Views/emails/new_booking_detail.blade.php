...
<div class="b-table-wrap">
    ...
    @php $ticket_types = $booking->getJsonMeta('ticket_types')
        ...
        @endif 
        */ ?>
         @php $ticket_types = $booking->getJsonMeta('selected_tickets')
        @endphp
        ...
        <tr>
            ...
            <td class="val no-r-padding">
                <table class="pricing-list" width="100%">
                    ...
                    */ ?>
                    <tr>
                            <td colspan="2" class="label-title"><strong>{{__("Ticket Prices:")}}</strong></td>
                        </tr>
                    @if(!empty($ticket_types))
                        @foreach($ticket_types as $type)
                        <?php 
                            $timeslot =  getArrayByColumn($type['timeslots'], 'id', $type['timeslot_id']);
                         ?>
                            <tr>
                                <td class="label">Ticket:  <strong>{{$type['title']}}</strong></td>
                                <td class="val no-r-padding">
                                    <strong>Time Slot: {{$timeslot['time']}}</strong>
                                </td>
                            </tr>
                            <tr>
                                <td class="label">Adult: {{$type['adult_ticket']}} * {{format_money($type['adult_price'])}}</td>
                                <td class="val no-r-padding">
                                    <strong>{{format_money($type['adult_price'] * $type['adult_ticket'])}}</strong>
                                </td>
                            </tr>
                            <tr>
                                <td class="label">Child: {{$type['child_ticket']}} * {{format_money($type['child_price'])}}</td>
                                <td class="val no-r-padding">
                                    <strong>{{format_money($type['child_price'] * $type['child_ticket'])}}</strong>
                                </td>
                            </tr>
                        @endforeach
                    @endif
