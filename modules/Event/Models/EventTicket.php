...
class EventTicket extends Bookable
{
    use SoftDeletes;
    protected $table = 'bravo_event_tickets';
    public $type = 'event_tickets';
    ...
    protected $seo_type = 'event_tickets';
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->ticketTimeSlotClass = TicketTimeSlot::class;
    ...
    public function timeslots(){
        return $this->hasMany($this->ticketTimeSlotClass, "parent_id")->orderBy('id','ASC');
    }
...
//                {
    ...
    public function isCustomeAvailableAt($filters = []){
    ...
    public function getCustomDatesInRange($start_date,$end_date, $type)
    {

        $query = $this->roomDateClass::query();
        $query->where('target_id',$this->id);
        if ($type == 'single') {
            $query->where('start_date','like','%'.date('Y-m-d H:i:s',strtotime($start_date)).'%');
            $customPrice = $query->groupBy('target_id')->first();
           $this->price = (!empty($customPrice) ? $customPrice->price : $this->price);
           return true;
        }else {
          $query->where('start_date','>=',date('Y-m-d H:i:s',strtotime($start_date)));
          $query->where('end_date','<=',date('Y-m-d H:i:s',strtotime($end_date)));
