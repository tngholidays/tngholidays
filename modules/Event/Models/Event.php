...
class Event extends Bookable
{
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->eventTicketClass = EventTicket::class;
    ...
    public function tickets(){
        return $this->hasMany($this->eventTicketClass, "parent_id");
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $selected_tickets_input = $request->input('selected_tickets');
        ...
        if (!empty($selected_tickets_input) && count($selected_tickets_input) > 0) {
            foreach ($selected_tickets_input as $key => $ticket) {
                $adult_price = (double)$ticket['adult_price'] * intval($ticket['adult_ticket']);
                $child_price = (double)$ticket['child_price'] * intval($ticket['child_ticket']);
                $total += $adult_price + $child_price;
                $selected_tickets[] = $ticket;
                $total_tickets += (int)$ticket['adult_ticket'];
                $total_tickets += (int)$ticket['child_ticket'];
            }
        }
        ...
        if (empty($total_tickets)) {
            return $this->sendError(__("Please select ticket and adult/child!"));
        }
        ...
        if ($check) {
            ...
            $booking->addMeta('selected_tickets', $selected_tickets);
    ...
    public function getBookingData()
    {
        $booking_data = [
            ...
            'ticket_types'    => [],
            'extra_price'     => [],
            'event_sale_price'     => $this->sale_price,
            ...
            'all_tickets'=>$this->getAllTickets($this),
    ...
    public static function getAllTickets($data)
    {
        $tickets = [];
        if (count($data->tickets) > 0) {
            foreach ($data->tickets as $key => $ticket) {
                ...
                $tick['sale_price'] = $ticket->sale_price;
                $tick['adult_ticket'] = 0;
                $tick['child_ticket'] = 0;
                $tick['adult_price'] = (isset($ticket->timeslots[0]) ? $ticket->timeslots[0]->adult_price : $ticket->sale_price);
                $tick['child_price'] = (isset($ticket->timeslots[0]) ? $ticket->timeslots[0]->child_price : $ticket->sale_price);
                $tick['timeslot_id'] = (isset($ticket->timeslots[0]) ? $ticket->timeslots[0]->id : null);

                $slots = [];
                if (count($ticket->timeslots) > 0) {
                    foreach ($ticket->timeslots as $key => $timeslot) {
                        $time['id'] =  $timeslot->id;
                        $time['time'] =  date('h:i A', strtotime($timeslot->time));
                        $time['adult_price'] =  $timeslot->adult_price;
                        $time['child_price'] =  $timeslot->child_price;
                        $slots[] = $time;
                    }
                }
                $tick['timeslots'] = $slots;
                $tickets[] = $tick;
            }
        }
        return $tickets;
    ...
    {
        $ticket = $this->eventTicketClass::find($id);
        return $ticket;
    ...
    public function getExtraContentArray(){
        $result = [];
        if (!empty($this->extra_content) && count($this->extra_content) > 0) {
            foreach ($this->extra_content as $key => $extra) {
                $result[$extra['type']] = $extra;
            }
        }
        return $result;
    }
